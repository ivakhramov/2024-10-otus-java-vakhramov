...
public class MessageController {
    ...
    private static final String ROOM_1408_TOPIC = "/topic/response.1408";
    ...
    public void getMessage(@DestinationVariable("roomId") String roomId, Message message) {
        ...
        template.convertAndSend(ROOM_1408_TOPIC, escapedMessage);
    }
    ...
    private String parseRoomId(String simpDestination) {
        try {
            ...
            logger.error("Can not get roomId from destination: {}", simpDestination, ex);
    ...
    private Flux<Message> getMessagesByRoomId(String roomId) {
        String uri = roomId.equals("1408") ? "/msg/all" : String.format("/msg/%s", roomId);
        return datastoreClient
                .get()
                .uri(uri)
